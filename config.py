...
class Settings:
    ...
    # автоматически находить задачи во входящих сообщениях.
    # Доп. вызов LLM делается ТОЛЬКО если в тексте есть маркер дела (см. _looks_like_task в bot.py),
    # а не на каждое сообщение — иначе "ок"/"спс"/"привет" тоже жгли бы токены.
    ...
    # A/B тест стиля: если true — генерирует ДВА варианта черновика (= двойной расход токенов).
    # Включается только вручную через /abon, по умолчанию выключен.
    ab_test_mode: bool = os.getenv("AB_TEST_MODE", "false").lower() == "true"

    # сколько сообщений должно накопиться в чате, чтобы запустить генерацию профиля (legacy, не используется в горячем пути)
    tone_update_interval: int = int(os.getenv("TONE_UPDATE_INTERVAL", "5"))

    # --- экономия токенов: размеры контекста, уходящего в LLM ---
    # Главные рычаги расхода. Чем меньше — тем дешевле каждый ответ. Дефолты подобраны так,
    # чтобы качество ответа не просело, а лимит Groq растянулся в несколько раз.

    # сколько последних сообщений чата передавать в LLM при генерации ответа (было жёстко 30)
    reply_history_limit: int = int(os.getenv("REPLY_HISTORY_LIMIT", "10"))
    # сколько твоих прошлых ответов именно этому контакту подмешивать для персонализации (было 15)
    contact_context_limit: int = int(os.getenv("CONTACT_CONTEXT_LIMIT", "6"))
    # сколько одобренных A/B-вариантов подмешивать в примеры стиля (было 10)
    style_feedback_limit: int = int(os.getenv("STYLE_FEEDBACK_LIMIT", "4"))
    # сколько сообщений брать для генерации AI-профиля контакта (было 30)
    summary_context_limit: int = int(os.getenv("SUMMARY_CONTEXT_LIMIT", "20"))
